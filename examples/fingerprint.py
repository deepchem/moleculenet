...
def load_model(args, tasks, hyperparams):
    if args['model'] == 'RF':
        model = dc.models.SingletaskToMultitask(tasks, partial(rf_model_builder, hyperparams=hyperparams))
    else:
...
def main(save_path, args, hyperparams):
    ...
    with open(save_path + '/eval.txt', 'w') as f:
        f.write('Best val {}: {:.4f} +- {:.4f}\n'.format(
            args['metric'], np.mean(all_run_val_metrics), np.std(all_run_val_metrics)))
        f.write('Test {}: {:.4f} +- {:.4f}\n'.format(
            args['metric'], np.mean(all_run_test_metrics), np.std(all_run_test_metrics)))
...
def bayesian_optimization(args):
    ...
    def objective(hyperparams):
        ...
        results.append((save_path, val_metric_to_minimize, val_metrics, test_metrics))

        return val_metric_to_minimize

    fmin(objective, candidate_hypers, algo=tpe.suggest, max_evals=args['num_trials'])
    results.sort(key=lambda tup: tup[1])
    best_trial_path, _, best_val_metrics, best_test_metrics = results[0]

    copyfile(best_trial_path + '/configure.json', args['result_path'] + '/configure.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', choices=['BACE'], default='BACE',
                        help='Dataset to use (default: BACE)')
    parser.add_argument('-m', '--model', choices=['RF'], default='RF',
                        help='Options include 1) random forest (RF) (default: RF)')
    parser.add_argument('-f', '--featurizer', choices=['ECFP'], default='ECFP',
    ...
    if args['hyper_search']:
        ...
        val_metrics, test_metrics = main(args['result_path'], args, default_hyperparams)
