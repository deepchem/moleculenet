...
def load_qm9(featurizer='CoulombMatrix',
  ...
  if featurizer in ['CoulombMatrix', 'BPSymmetryFunctionInput', 'MP', 'Raw']:
    ...
    if not os.path.exists(dataset_file):
      moleculenet.utils.download_url(url=GDB9_URL, dest_dir=data_dir)
      moleculenet.utils.untargz_file(
          os.path.join(data_dir, 'gdb9.tar.gz'), data_dir)
  else:
    dataset_file = os.path.join(data_dir, "qm9.csv")
    if not os.path.exists(dataset_file):
      moleculenet.utils.download_url(url=QM9_CSV_URL, dest_dir=data_dir)

  if featurizer in ['CoulombMatrix', 'BPSymmetryFunctionInput', 'MP', 'Raw']:
    if featurizer == 'CoulombMatrix':
      featurizer = moleculenet.featurizers.CoulombMatrix(29)
    elif featurizer == 'BPSymmetryFunctionInput':
      featurizer = moleculenet.featurizers.BPSymmetryFunctionInput(29)
    elif featurizer == 'Raw':
  ...
  splitters = {
      ...
      'stratified': moleculenet.splitters.SingletaskStratifiedSplitter(task_number=11)
  }
